--crea tabla last_aisle_scoring_by_partner
...
SELECT 
  ...
  ARRAY_AGG(
    STRUCT(
      remote_product_id,
      product_name,
      IFNULL(e.score, new_score) AS product_score
    )
    ORDER BY IFNULL(e.score, new_score) DESC
    LIMIT 50
  ) AS top_50_products
  FROM final dv
  LEFT JOIN EXT AS e 
  ON CONCAT(dv.remote_product_id, dv.remote_vendor_id) = e.clave
  GROUP BY 1, 2
--) GROUP BY 1 , 2 , 3,4
